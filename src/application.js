...
export default GObject.registerClass(
  ...
  class Application extends Gtk.Application {
    _init({ version }) {
      ...
      this.set_option_context_parameter_string(
      ...
      this.connect("open", (application, files, hint) => {
        ...
        const isGitCommitMessage = this.commitMessageFilePath.includes(
          "COMMIT_EDITMSG",
        );
        const isTestCommitMessage =
          this.commitMessageFilePath.includes("tests/message-with-body") ||
          this.commitMessageFilePath.includes("tests/message-without-body");
        ...
        const isTestMergeMessage = this.commitMessageFilePath.includes(
          "tests/merge",
        );
        this.isMergeMessage = isMergeMessage || isTestMergeMessage;

        // Git tag message.
        const isGitTagMessage = this.commitMessageFilePath.includes(
          "TAG_EDITMSG",
        );
        const isTestTagMessage = this.commitMessageFilePath.includes(
          "tests/tag-message",
        );
        this.isTagMessage = isGitTagMessage || isTestTagMessage;

        // AddP Hunk Edit message.
        const _isAddPHunkEditMessage = this.commitMessageFilePath.includes(
          "addp-hunk-edit.diff",
        );
        const _isTestAddPHunkEditMessage = this.commitMessageFilePath.includes(
          "tests/add-p-edit-hunk",
        );
        this.isAddPHunkEditMessage =
          _isAddPHunkEditMessage || _isTestAddPHunkEditMessage;

        // Rebase message.
        const _isRebaseMessage = this.commitMessageFilePath.includes(
          "rebase-merge/git-rebase-todo",
        );
        const _isTestRebaseMessage = this.commitMessageFilePath.includes(
          "tests/rebase",
        );
        ...
        try {
          ...
          if (this.isAddPHunkEditMessage) {
            commitMessage = commitMessage.substr(
          ...
          if (this.isCommitMessage) {
            ...
            print(
              `Warning: unknown Git commit type encountered in: ${this.commitMessageFilePath}`,
            );
          }

          // Set the title.
          this.active_window.set_title(
        ...
        startOfText =
          commitBody.length > 0
            ? this.buffer.get_iter_at_offset(commitBody.length)
      ...
      this.connect("startup", () => {
        ...
        const highlightText = () => {
          ...
          const endOfFirstLineIterator = this.buffer.get_iter_at_offset(
            firstLineLength,
          );
          ...
          this.buffer.remove_tag_by_name(
            HIGHLIGHT_BACKGROUND_TAG_NAME,
            startOfTextIterator,
            endOfTextIterator,
          );
          ...
          if (firstLineLength > FIRST_LINE_CHARACTER_LIMIT) {
            const startOfOverflowIterator = this.buffer.get_iter_at_offset(
        ...
        this.buffer.connect("end-user-action", () => {
          ...
          if (this.lastActionWasSelectAll) {
            ...
            const cursorIterator = this.buffer.get_iter_at_offset(
          ...
          if (
            ...
            numberOfLinesInCommitMessage >
              this.previousNumberOfLinesInCommitMessage
          ) {
            // Delete the newline
            this.buffer.backspace(
              /* iter: */ this.buffer.get_iter_at_offset(
                this.buffer.cursor_position,
              ),
              /* interactive: */ true,
          ...
          if (
            ...
            cursorPosition === firstLineLength + 1 &&
            numberOfLinesInCommitMessage ===
              this.numberOfLinesInCommitComment + 3 &&
            /* and person didn’t reach here by deleting existing content */
            numberOfLinesInCommitMessage >
              this.previousNumberOfLinesInCommitMessage
          ) {
            ...
            const newline = "\n";
            this.buffer.insert_interactive_at_cursor(
              newline,
              newline.length,
        ...
        this.messageText.connect("select-all", (textView, selected) => {
          if (selected) {
            ...
            setTimeout(() => {
              ...
              const selectEndIterator = this.buffer.get_iter_at_offset(
        ...
        this.commitButton.connect("clicked", () => {
          ...
          const textToSave = this.buffer.text;
    ...
    validateCommitButton() {
      ...
      const numberOfLinesInMessageExcludingComments =
        numberOfLinesInCommitMessage - this.numberOfLinesInCommitComment;
